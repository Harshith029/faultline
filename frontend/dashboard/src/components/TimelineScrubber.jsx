export default function TimelineScrubber({ activeWindow, onChange, windows }) {
  const getPhaseConfig = (windowData) => {
    if (windowData?.triggered || windowData?.outage) {
      return {
        label: 'Cascade zone',
    ...
    const anyElevated = windowData && Object.values(windowData.metrics ?? {}).some((z) => z >= 2)
    if ((windowData?.signal_count ?? 0) > 0 || anyElevated) {
      return {
  ...
  const phase = getPhaseConfig(activeData)
  const dense = windows.length > 16
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="timeline-shell rounded-[30px] border border-white/10 p-6 shadow-inner shadow-white/5">
        ...
        <div className={`mt-6 flex items-start justify-between ${dense ? 'gap-0.5' : 'gap-2'}`}>
          {windows.map((window) => {
            const isActive = window.window_number === activeWindow
            const windowPhase = getPhaseConfig(window)
            const showLabel = !dense || isActive || window.window_number % 5 === 0

            return (
              <div key={window.window_number} className="flex min-w-0 flex-1 flex-col items-center gap-2">
                <div
                  className={`rounded-full transition-all duration-300 ${dense ? 'h-2 w-2' : 'h-3 w-3'} ${isActive ? 'scale-125' : ''}`}
                  style={{
                    backgroundColor: isActive ? windowPhase.color : 'rgba(148, 163, 184, 0.32)',
                    boxShadow: isActive ? `0 0 22px ${windowPhase.color}77` : 'none',
                  }}
                />
                <div className={`text-[10px] ${isActive ? 'font-semibold text-white' : 'text-slate-500'} ${showLabel ? '' : 'invisible'}`}>
                  {dense ? window.window_number : `W${window.window_number}`}
                </div>
                {!dense && (
