...
const WINDOW_INSIGHTS = [
  {
    ...
    severity: 'nominal',
    insight: 'All services are operating within expected parameters. Latency, retry rate, and error rate are moving inside their baseline envelope, with no qualified drift signals.',
  ...
  },
  {
    title: 'System Nominal',
    severity: 'nominal',
    insight: 'Telemetry remains healthy. Metrics are oscillating in both directions around baseline, which is the signature of ordinary variance rather than structural drift.',
    trajectoryText: 'Stable baseline',
    prediction: null,
  },
  {
    title: 'System Nominal',
    severity: 'nominal',
    insight: 'Latency and error rate show mild positive excursions this window, but nothing approaches the qualification threshold. The system remains inside its normal envelope.',
    trajectoryText: 'Stable baseline',
    prediction: null,
  },
  {
    ...
    severity: 'watching',
    insight: 'The baseline window is complete. Every metric is now being scored against its established mean and variance, so any sustained departure from here will register immediately.',
    trajectoryText: 'Reference frame locked',
    prediction: 'Detection sensitivity is now fully active for all three signals.',
  },
  {
    title: 'Threshold Crossed',
    severity: 'watching',
    insight: 'P99 latency has crossed 2 sigma for the first time, reaching z=2.4. It is still a single-window event, so the system remains in watch mode while persistence is evaluated.',
    trajectoryText: 'First excursion',
    prediction: 'A second consecutive elevated window will qualify latency as the first active signal.',
  },
  {
    title: 'Signal 1 Qualified',
    severity: 'warning',
    insight: 'Latency has sustained above threshold for two consecutive windows and is now qualified at z=2.9. This is the first concrete sign of structural instability rather than transient variance.',
  ...
  },
  {
    ...
    severity: 'warning',
    insight: 'Latency is holding at three sustained windows while retry rate has crossed 2 sigma for the first time at z=2.4. Retries compound upstream pressure, so this pairing is the classic pre-cascade signature.',
    trajectoryText: 'Amplification loop forming',
    prediction: 'If retry sustains one more window it qualifies, and the convergence score will jump toward the trigger threshold.',
  },
  {
    title: 'Incident Detected',
    severity: 'triggered',
    insight: 'FAULTLINE has triggered. Retry rate qualified alongside latency, and with two sustained signals converging the R score jumped to 3.96 — past the 3.0 threshold. Error rate is elevated for the first time and expected to follow.',
    trajectoryText: 'Detection active',
    prediction: 'Error rate qualifies next window, and downstream dependencies begin entering the blast radius.',
  },
  {
    title: 'Full Convergence',
    severity: 'triggered',
    insight: 'Error rate has qualified, so all three signals are now sustained and converging. The cascade is validated end to end and the risk score is climbing steeply.',
  ...
  },
  {
    ...
    severity: 'triggered',
    insight: 'The error rate has now breached its 2% SLO — the moment a static threshold alert would fire for the first time, three windows after FAULTLINE detected the cascade. Intervention now primarily reduces outage duration.',
  ...
  },
  {
    ...
    severity: 'outage',
    insight: 'The modeled cascade has become a confirmed outage. The lead time between detection at W8 and this window was the difference between prevention and active recovery.',
